...
function DenmarkStraitPage() {
  ...
  const [torlesFolyamatban, setTorlesFolyamatban] = useState('')
  ...
  useEffect(() => {
    const loadResztvevok = async () => {
      try {
        ...
        const lehetsegesNevek = [CSATA_NEV, 'Denmark Strait csata']
        let sikeresValasz = false

        for (const nev of lehetsegesNevek) {
        ...
        if (!sikeresValasz) {
          throw new Error('A csata adatainak lekérése sikertelen.')
        }
      } catch (err) {
  ...
  const handleTorles = async (hajoNev) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(szerverUzenet || 'A hajó törlése sikertelen.')
      }
              ...
              <article className="card h-100 border-0 shadow-sm battle-card">
                <div className="card-body d-flex align-items-center justify-content-between gap-3">
                  ...
                  <button
                    ...
                    disabled={torlesFolyamatban === nev}
